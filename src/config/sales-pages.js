...
const input = (key, label, required = false, rules = {}) => ({ key, label, type: 'input', required, ...rules })
const phone = (key, label, required = false) => input(key, label, required, { format: 'mobile', inputMode: 'numeric', maxLength: 11 })
const select = (key, label, options, required = false, meta = {}) => ({ key, label, type: 'select', options, required, ...meta })
...
const textarea = (key, label, required = false, maxLength = 500) => ({ key, label, type: 'textarea', required, maxLength })
const number = (key, label, required = false, precision = 2, min = 0, max = 99999999) => ({ key, label, type: 'number', required, precision, min, max })
...
const saleBaseFields = [
  ...
  phone('mobile', '手机号码'), select('store', '销售分店', stores, true), date('saleDate', '销售日期', 'date', true),
...
export const salesPageConfigs = {
  '合同管理': {
    ...
    formFields: [
      select('store', '签单门店', stores, true),
      input('customerName', '选择客户', true, { maxLength: 30 }), phone('mobile', '手机号'), select('idType', '证件类别', ['中国大陆居民身份证', '香港来往大陆通行证', '澳门来往大陆通行证', '台湾来往大陆通行证', '护照']), input('idNo', '证件号', false, { maxLength: 30 }),
      date('birthday', '客户生日'), input('age', '客户年龄'), input('address', '居住地址'), date('signedAt', '合同签订日期', 'date', true), input('salesperson', '选择签单人', true),
      select('roomType', '房间类型', ['大床房', '小套房', '特价房', '套房', 'VIP302', 'VIP512', '一房一厅', '女王套房', '总统套房', '待甲方确认']), select('nursingType', '护理类型', ['基础护理', '护士团队', '7天一对一', '7天一对一+21天团队护理', '双师护航', '产后修复', '修养护理', '专属护理', '待甲方确认'], true), input('occupation', '客户职业'), date('dueDate', '客户预产期'),
      ...
      switchField('firstOrder', '是否首单'), switchField('companionMeal', '是否有陪护餐'), switchField('enabled', '启用当前合同'), select('packageSelection', '合同套餐', [], false, { dependsOn: 'store', catalogSelector: 'package' }),
  ...
  '套餐管理': {
    key: 'packages', icon: 'el-icon-box', description: '已录入甲方提供的7类套餐及28/35/42/56天价格版本，同时保留原价、活动价和成交价。',
    ...
    defaultFilters: {},
    columns: [
      col('packageNo', '套餐编号', 145), col('packageDisplayName', '套餐名称', 210), col('packageDays', '入住天数', 95),
      col('nursingType', '护理方式', 120), col('roomType', '适用房型', 120),
      col('originalPrice', '套餐原价', 105, false, true), col('activityPrice', '活动价', 105, false, true),
      col('dealPrice', '成交价', 105, false, true), col('effectiveDate', '价格生效日', 115),
      col('auditStatus', '审核状态', 105, true), col('enabled', '是否启用', 95, true),
      col('store', '所属分店', 140), col('dataStatus', '资料状态', 220)
    ],
    formFields: [
      select('store', '所属门店', stores, true),
      select('packageSelection', '套餐名称', [], true, { dependsOn: 'store', catalogSelector: 'package' }),
      number('packageDays', '套餐天数', true, 0, 1, 365),
      select('nursingType', '护理方式', ['未注明', '基础护理', '护士团队', '7天一对一', '7天一对一+21天团队护理', '双师护航', '产后修复', '修养护理', '专属护理'], true),
      number('originalPrice', '套餐原价', true, 2, 0.01, 10000000),
      number('activityPrice', '活动价', true, 2, 0.01, 10000000), number('dealPrice', '成交价', true, 2, 0.01, 10000000),
      select('roomType', '适用房型', ['大床房', '小套房', '特价房', '套房', 'VIP302', 'VIP512', 'VIP3楼', 'VIP5楼', '待甲方确认']),
      date('effectiveDate', '价格生效日期'), input('versionNo', '价格版本'),
      textarea('details', '套餐权益说明'), textarea('roomInfo', '房型说明')
    ],
  ...
  '优惠管理': {
    ...
    actions: ['添加', '编辑', '删除', '审核', '核销', '停用', '导出'],
    filters: [input('customerName', '客户姓名', false, { maxLength: 30 }), phone('mobile', '手机号'), select('couponType', '优惠券类型', discountCouponTypes), select('store', '所属分店', stores), select('auditStatus', '审核状态', ['已通过', '待审核', '审核不通过']), select('status', '使用状态', ['未使用', '部分使用', '已核销', '已停用', '已过期']), dateRange('createdRange', '制单日期')],
    columns: [col('discountNo', '编号', 135), col('customerName', '客户姓名'), col('mobile', '手机号', 125), col('couponName', '优惠券名称', 150), col('couponType', '优惠券类型'), col('itemType', '优惠项目类型', 120), col('quantity', '数量'), col('couponAmount', '优惠券金额', 110, false, true), col('usedAmount', '已核销金额', 105, false, true), col('remainingAmount', '剩余金额', 100, false, true), col('deadline', '截止日期', 120), col('auditStatus', '审核状态', 105, true), col('status', '使用状态', 95, true), col('saleNo', '关联业务单号', 145), col('usedAt', '最近核销时间', 150), col('auditor', '审核人'), col('auditRemark', '审核意见', 160), col('operationTrail', '操作流水', 220), col('remark', '备注', 170), col('creator', '制单人'), col('createdAt', '制单时间', 150), col('store', '分店', 140), col('disableReason', '停用说明', 170)],
    formFields: [input('customerName', '选择客户', true, { maxLength: 30 }), phone('mobile', '手机号', true), input('couponName', '优惠券名称', true, { maxLength: 50 }), number('couponAmount', '优惠券金额', true, 2, 0.01, 1000000), select('couponType', '优惠券类型', discountCouponTypes, true), select('store', '所属分店', stores, true), select('limitType', '限制消费类型', ['项目', '商品']), select('consumeType', '消费类型', ['产后类', '产康服务', '护理服务', '膳食服务', '物料商品']), input('consumeItem', '消费项目/商品', false, { maxLength: 100 }), select('validType', '使用类型', ['时间段', '有效期']), date('startsAt', '优惠开始时间'), date('endsAt', '优惠结束时间'), number('validDays', '有效期（天）', false, 0, 0, 3650), textarea('useRule', '使用规则'), switchField('receivePayment', '是否收款'), textarea('remark', '优惠券备注')],
    auditFields,
    metrics: ['优惠记录', '待审核', '未使用', '剩余金额']
  },
  '优惠券管理': {
    key: 'coupons', icon: 'el-icon-tickets', description: '配置可发行优惠券的额度、数量、领用限制、适用范围和发放形式。',
    actions: ['添加', '编辑', '删除', '分发', '导出'],
    filters: [input('couponName', '优惠券名称'), select('couponType', '优惠券类型', couponTypes), select('store', '所属分店', stores)],
    columns: [col('couponNo', '优惠券编码', 140), col('couponName', '优惠券名称', 150), col('couponType', '优惠券类型'), col('itemType', '优惠项目类型', 120), col('couponAmount', '优惠券金额', 110, false, true), col('store', '所属门店', 140), col('startsAt', '开始时间', 150), col('endsAt', '结束时间', 150), col('totalQuantity', '总数量'), col('issuedQuantity', '发放数量'), col('remainingQuantity', '剩余可发'), col('limitPerCustomer', '每客户限领数量', 130), col('status', '发行状态', 100, true), col('creator', '制单人'), col('createdAt', '制单时间', 150)],
    formFields: [input('couponName', '优惠券名称', true, { maxLength: 50 }), number('couponAmount', '优惠券金额', true, 2, 0.01, 1000000), number('totalQuantity', '优惠券数量', true, 0, 1, 100000), select('couponType', '优惠券类型', couponTypes, true), select('store', '所属分店', stores, true), select('limitType', '限制消费类型', ['项目', '商品']), select('consumeType', '消费类型', ['产后类', '产康服务', '护理服务', '膳食服务', '物料商品']), input('consumeItem', '消费项目/商品', false, { maxLength: 100 }), date('startsAt', '优惠开始时间', 'date', true), date('endsAt', '优惠结束时间', 'date', true), number('limitPerCustomer', '单客户可领用数量', true, 0, 1, 100), switchField('stackable', '是否叠加'), select('scope', '适用范围', ['所有人', '会员', '非会员']), select('sendType', '发放形式', ['自由领取', '店内发放']), textarea('remark', '优惠券备注')],
...
const salesFeatureAliases = {
  '合同与销售': {
    source: '合同管理',
    featureId: 'F020',
    title: '合同与销售',
    description: '贯通客户签约、套餐选择、合同审核、收款状态和入住安排；查询与操作均受门店权限约束。'
  },
  '套餐卡/次卡管理': {
    source: '卡类套餐',
    featureId: 'F082',
    title: '套餐卡/次卡管理',
    key: 'card-packages',
    description: '管理套餐卡与次卡的开卡、次数、有效期和适用门店；卡资产扣减由会员资产流水留痕。'
  },
  电子合同: {
    featureId: 'F107',
    title: '电子合同',
    ...
    description: '管理电子合同模板、签署申请与归档状态；签署通道未启用时保留待签与失败原因，不显示为已签署。',
    actions: ['新增签署申请', '查看', '导出'],
    filters: [input('customerName', '客户姓名'), input('contractNo', '合同编号'), select('signStatus', '签署状态', ['待发起', '待签署', '已签署', '已撤销', '失败']), select('store', '所属门店', stores)],
    columns: [col('contractNo', '合同编号', 150), col('customerName', '客户姓名'), col('templateName', '合同模板', 160), col('store', '所属门店', 140), col('signStatus', '签署状态', 105, true), col('updatedAt', '更新时间', 150)],
    formFields: [select('store', '所属门店', stores, true), input('customerName', '客户姓名', true), input('templateName', '合同模板', true), textarea('remark', '签署说明')],
    lineColumns: [],
    metrics: ['签署申请', '待签署', '已签署', '失败记录'],
    integrationOnly: true
  }
}

export function getSalesPageConfig(title) {
  const alias = salesFeatureAliases[title]
  if (alias && alias.source) return { ...salesPageConfigs[alias.source], ...alias }
  if (alias) return alias
  return salesPageConfigs[title] || salesPageConfigs['合同管理']
}

export function getSalesFeatureAlias(title) {
  return salesFeatureAliases[title] || null
}
