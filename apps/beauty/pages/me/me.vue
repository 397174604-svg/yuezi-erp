...
export default {
  ...
  methods: {
    async doLogout() {
      await logoutRemote()
      // #ifdef H5
      location.href = (import.meta.env && import.meta.env.VITE_PORTAL_URL) || ('http://' + (location.hostname || '127.0.0.1') + ':5179/') // 门户地址优先构建期 VITE_PORTAL_URL,兜底本地 :5179
      // #endif
    },
    open(m) {
      if (m.mg) uni.navigateTo({ url: '/pages/manage/manage?key=' + m.mg })
      else if (m.k) uni.navigateTo({ url: '/pages/sub/sub?key=' + m.k })
