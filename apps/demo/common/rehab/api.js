...
export function createApi(cfg) {
  ...
  return {
    ...
    listMemberLevels: async () => unwrap(await cfg.transport('GET', base + '/api/v1/member-levels', { headers: hdr() })),
    setMemberLevelDiscount: async (name, discount) => unwrap(await cfg.transport('PATCH', base + '/api/v1/member-levels/' + encodeURIComponent(name), { headers: hdr(), body: JSON.stringify({ discount }) })),
