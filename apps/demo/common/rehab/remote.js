/* 奇德芬芳 · 产康门店端 — 数据访问层（纯后端，无 mock）
 * 数据全部来自后端：门店/经营/预约/技师/品项/客户/收银等。经共享客户端 api.js（uniTransport）打中台。
...
export async function loadDashboard() {
  ...
  try {
    ...
    const [stats, appts, items, stores, staff, customers, levels] = await Promise.all([
      api.getBusinessStats({}), api.listAppointments({}), api.listItems({ status: '启用' }),
    ...
    return {
      ...
      techs: (staff || []).filter(s => (s.role === '技师' || s.role === '护士') && (s.status || '在职') === '在职').slice(0, 12).map(s => ({ staffId: s.staff_id, name: s.name, status: s.status || '在职' })),
      cashierCustomer,
      discountMap,
