<template>
  <view class="screen">
    <view class="topbar"><text class="nm">奇德芬芳 · 产康</text></view>
    <scroll-view scroll-y class="scroll">
      <view class="pad">
        ...
        <text class="foot">奇德芬芳 · 产康门店端 · uni-app 参考实现</text>
        <!-- #ifdef H5 -->
  ...
  <demo-tabbar end="rehab" active="me/me" />
</template>

<script>
import { loadDashboard, logoutRemote, REMOTE } from '@/common/rehab/remote.js'
export default {
  ...
  methods: {
    ...
    open(m) {
      if (m.mg) uni.navigateTo({ url: '/pages/rehab/manage/manage?key=' + m.mg })
      else if (m.k) uni.navigateTo({ url: '/pages/rehab/sub/sub?key=' + m.k })
