<template>
  <view class="screen">
    ...
    <view class="topbar">
      <view class="brand">
        <text class="logo">☾</text><text class="nm">奇德芬芳 · 产康</text>
      </view>
  ...
  <demo-tabbar end="rehab" active="home/home" />
</template>

<script>
import { loadDashboard } from '@/common/rehab/remote.js'
export default {
  data() {
  ...
  methods: {
    toCashier() { uni.reLaunch({ url: '/pages/rehab/cashier/cashier' }) },
    toCustomers() { uni.reLaunch({ url: '/pages/rehab/customers/customers' }) },
    toSub(key) { uni.navigateTo({ url: '/pages/rehab/sub/sub?key=' + key }) }
  }
}
</script>
