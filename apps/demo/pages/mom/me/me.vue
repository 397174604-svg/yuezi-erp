<template>
    ...
    <scroll-view scroll-y class="scroll">
      <view class="pad">
        ...
        <view class="prof"><view class="big">{{ me.avatar }}</view>
          <view><text class="pn">{{ me.name }}</text><text class="ps">臻享 28 天套餐 · 房间 {{ me.room }}</text></view></view>
  ...
  <demo-tabbar end="mom" active="me/me" />
</template>
<script>
import { MEMENU } from '@/common/mom/data.js'
import { loadDashboard, logoutRemote } from '@/common/mom/remote.js'
export default {
  ...
  methods: {
    async doLogout() {
      ...
      // #endif
    }, open(m) { if (m.page) uni.navigateTo({ url: m.page }); else if (m.k) uni.navigateTo({ url: '/pages/mom/sub/sub?key=' + m.k }); else uni.showToast({ title: m.l + '（建设中）', icon: 'none' }) } }
}
</script>
