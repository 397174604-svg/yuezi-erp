<template>
  <view class="screen">
    ...
    <view class="topbar">
      <view class="brand">
        <text class="logo">☾</text><text class="nm">奇德芬芳 · 科颜</text>
      </view>
  ...
  <demo-tabbar end="beauty" active="home/home" />
</template>

<script>
import { loadDashboard } from '@/common/beauty/remote.js'
export default {
  data() {
    const m = getApp().globalData.data
    return { store: m.store, kpis: m.kpis, appts: m.appointments, techs: m.techs }
  },
  async onLoad() {
  ...
  methods: {
    toCashier() { uni.reLaunch({ url: '/pages/beauty/cashier/cashier' }) },
    toCustomers() { uni.reLaunch({ url: '/pages/beauty/customers/customers' }) },
    toSub(key) { uni.navigateTo({ url: '/pages/beauty/sub/sub?key=' + key }) }
  }
}
</script>
