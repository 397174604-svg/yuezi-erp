...
export function createApi(cfg) {
  ...
  return {
    ...
    updateCustomerProfile: async (id, input) => unwrap(await cfg.transport('PATCH', base + '/api/v1/customers/' + id + '/profile', { headers: hdr(), body: JSON.stringify(input) })),
    ...
    getBrand: async () => unwrap(await cfg.transport('GET', base + '/api/v1/brand', { headers: hdr() })),
    ...
    chooseServiceRecommendation: async (recommendationId, input) => unwrap(await cfg.transport('POST', base + '/api/v1/service-recommendations/' + recommendationId + '/choices', { headers: hdr(), body: JSON.stringify(input) })),
    ...
    // M-A F073/F074（宝妈端只读）：宝宝列表 / 宝宝日志时间线（kind 可选过滤）/ 本人产后评估时间线
    listBabies: async (filter) => unwrap(await cfg.transport('GET', base + '/api/v1/babies' + qs(filter), { headers: hdr() })),
    getBabyTimeline: async (babyId, opt) => unwrap(await cfg.transport('GET', base + '/api/v1/babies/' + babyId + '/timeline' + qs(opt), { headers: hdr() })),
    getNursingTimeline: async (id) => unwrap(await cfg.transport('GET', base + '/api/v1/customers/' + id + '/nursing-timeline', { headers: hdr() })),
    ...
    mallPurchase: async (input, idempotencyKey) => unwrap(await cfg.transport('POST', base + '/api/v1/mall/purchase', { headers: idempotencyKey ? hdr({ 'idempotency-key': idempotencyKey }) : hdr(), body: JSON.stringify(input) })),
    ...
    customerPasswordLogin: async (phone, password) => unwrap(await cfg.transport('POST', base + '/api/v1/auth/customer/password-login', { headers: hdr(), body: JSON.stringify({ phone, password }) })),
    wechatPhoneLogin: async (loginCode, phoneCode) => unwrap(await cfg.transport('POST', base + '/api/v1/auth/customer/wechat-phone-login', { headers: hdr(), body: JSON.stringify({ loginCode, phoneCode }) })),
    getCustomerProfileContext: async () => unwrap(await cfg.transport('GET', base + '/api/v1/customer/profile-context', { headers: hdr() })),
    updateCustomerProfileContext: async (input) => unwrap(await cfg.transport('PATCH', base + '/api/v1/customer/profile-context', { headers: hdr(), body: JSON.stringify(input) })),
    createCustomerFamilyInvite: async (input = {}) => unwrap(await cfg.transport('POST', base + '/api/v1/customer/family-invites', { headers: hdr(), body: JSON.stringify(input) })),
    listCustomerFamilyBindings: async () => unwrap(await cfg.transport('GET', base + '/api/v1/customer/family-bindings', { headers: hdr() })),
    acceptCustomerFamilyInvite: async (input) => unwrap(await cfg.transport('POST', base + '/api/v1/customer/family-bindings', { headers: hdr(), body: JSON.stringify(input) })),
    listCustomerRoomTypes: async () => unwrap(await cfg.transport('GET', base + '/api/v1/customer/room-types', { headers: hdr() })),
    getCustomerRoomLayout: async (filter) => unwrap(await cfg.transport('GET', base + '/api/v1/customer/room-layout' + qs(filter), { headers: hdr() })),
    listCustomerRoomOrders: async () => unwrap(await cfg.transport('GET', base + '/api/v1/customer/room-orders', { headers: hdr() })),
    createCustomerRoomOrder: async (input) => unwrap(await cfg.transport('POST', base + '/api/v1/customer/room-orders', { headers: hdr(), body: JSON.stringify(input) })),
    cancelCustomerRoomOrder: async (bookingId) => unwrap(await cfg.transport('POST', base + '/api/v1/customer/room-orders/' + bookingId + '/cancel', { headers: hdr(), body: '{}' })),
    sandboxPayCustomerRoomOrder: async (bookingId) => unwrap(await cfg.transport('POST', base + '/api/v1/customer/room-orders/' + bookingId + '/sandbox-pay', { headers: hdr(), body: '{}' })),
    getCustomerRehabCatalog: async () => unwrap(await cfg.transport('GET', base + '/api/v1/customer/rehab-catalog', { headers: hdr() })),
    createCustomerDirectRequest: async (input) => unwrap(await cfg.transport('POST', base + '/api/v1/customer/direct-requests', { headers: hdr(), body: JSON.stringify(input) })),
    resolveQrContext: async (scene) => unwrap(await cfg.transport('GET', base + '/api/v1/qr-context' + qs({ scene }), { headers: hdr() })),
    createCustomerServiceRequest: async (input) => unwrap(await cfg.transport('POST', base + '/api/v1/customer-service-requests', { headers: hdr(), body: JSON.stringify(input) })),
    ...
    getCustomerAccount: async () => unwrap(await cfg.transport('GET', base + '/api/v1/auth/customer/account', { headers: hdr() })),
    setCustomerPassword: async (newPassword, currentPassword) => unwrap(await cfg.transport('POST', base + '/api/v1/auth/customer/set-password', { headers: hdr(), body: JSON.stringify({ newPassword, currentPassword }) })),
    acceptCustomerAgreements: async (version, source = 'mom-weixin') => unwrap(await cfg.transport('POST', base + '/api/v1/auth/customer/agreements', { headers: hdr(), body: JSON.stringify({ version, source }) })),
