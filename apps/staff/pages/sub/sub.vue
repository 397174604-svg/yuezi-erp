  ...
  <view class="screen">
    ...
    <scroll-view scroll-y class="scroll">
      <view class="pad">
        ...
        <!-- 订单筛选栏（产康支付/订单管理：按客户/订单号 + 支付方式 + 日期）-->
...
import { loadPage, pageAction } from '@/common/remote.js'
import { loadPage as loadRehabPage } from '@/common/rehab.js' // 产康板块二级屏数据层（scope=rehab 时走此，不与月子域混）
export default {
  data() { return { page: {}, key: '', scope: '', busy: false, flt: { q: '', payMethod: '', date: '' }, payMethods: ['现金', '刷卡', '微信支付', '支付宝', '储值卡'], loading: false } },
  async onLoad(q) {
    this.key = q.key
    this.scope = q.scope || '' // 'rehab' → 产康板块二级屏，数据取 common/rehab.js（品项/客户限产康域）
    this.page = PAGES[q.key] || { title: '建设中', type: 'list', columns: [], rows: [] }
    const live = this.scope === 'rehab' ? await loadRehabPage(q.key, this.flt) : await loadPage(q.key)
  ...
  methods: {
    ...
    async applyFilter() { // 仅产康支付/订单管理屏用（filterable）
      if (this.loading) return
      this.loading = true
      const live = this.scope === 'rehab' ? await loadRehabPage(this.key, this.flt) : await loadPage(this.key, this.flt)
